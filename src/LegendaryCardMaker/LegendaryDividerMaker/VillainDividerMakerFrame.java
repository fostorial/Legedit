...
public class VillainDividerMakerFrame extends JDialog {
	...
	VillainDividerMakerToolbar hmt;
	
	boolean templateMode = false;
	
	public VillainDividerMakerFrame(Villain hc, boolean horizontal)
	{
		if (hc == null)
		{
			hc = new Villain();
		...
		hm = new VillainDividerMaker(hc, horizontal);
		...
		*/
		...
		else
		{
			layers.addMouseListener(new MouseListener() {
				...
				public void mouseReleased(MouseEvent e) {
					...
					hm.villain.imageOffsetX += e.getX() - startX;
					hm.villain.imageOffsetY += e.getY() - startY;
					
					System.out.println("Released: " + hm.villain.imageOffsetX + ":" + hm.villain.imageOffsetY);
		...
		hmt = new VillainDividerMakerToolbar(hm, this);
