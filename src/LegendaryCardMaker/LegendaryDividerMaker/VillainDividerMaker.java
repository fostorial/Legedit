...
public class VillainDividerMaker extends DividerMaker {
	...
	{	
	    ...
	    if (villain.cards != null && villain.cards.size() > 0)
	    {
        	...
        	if (teamIcon != null && teamIcon.getImagePath() != null)
        	{
        		...
        		BufferedImage bi = getIcon(villain.cards.get(0).cardTeam, teamMaxWidth, teamMaxHeight);
	    ...
	    if (villain.name != null)
	    {
	        ...
	        int stringLength = SwingUtilities.computeStringWidth(metrics, villain.name.toUpperCase());
	        int heroLength = width - heroNameStart - heroNameStart;
	        int x = heroNameStart + ((heroLength / 2) - (stringLength / 2));
	        
	        g2.drawString(villain.name.toUpperCase(), x, heroNameY);
	...
	public void exportHeroImage(BufferedImage image)
	{
		try
		{
			...
			if (exportToPNG)
			{
				File newFile = new File(exportFolder + File.separator + "divider_" + villain.name + ".png");
				exportToPNG(image, newFile);
			}
			else
			{
				File newFile = new File(exportFolder + File.separator + "divider_" + villain.name + ".jpg");
	...
	private Icon getTeamIcon()
	{
    	...
    	for (VillainCard hc : villain.cards)
    	{
